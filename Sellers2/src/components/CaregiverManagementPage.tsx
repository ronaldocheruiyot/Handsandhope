    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      {addDialogOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 overflow-y-auto">
          <Card className="w-full max-w-2xl max-h-[95vh] overflow-y-auto my-8 shadow-2xl">
            ...
            <CardContent className="p-6">
              <form onSubmit={(e) => { e.preventDefault(); handleAddCaregiver(); }} className="space-y-6">
                {/* Full Name Field */}
                ...
                <div className="space-y-4 border-t pt-4">
                  <h4 className="font-semibold">Permission Level *</h4>
                  <div className="grid gap-3">
                    {(Object.keys(permissionPresets) as Array<keyof typeof permissionPresets>).map((preset) => {
                      ...
                      return (
                        <Card
                          key={preset}
                          className={`p-4 cursor-pointer transition-all ${newCaregiver.permissionLevel === preset
              ...
              </form>              <div className="space-y-2"> {/* Relationship */}
                <Select>...</Select>
              </div> {/* Properly closed */}
            </CardContent>
          </Card>
      ...
      {editDialogOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 overflow-y-auto">
          <Card className="w-full max-w-2xl max-h-[95vh] overflow-y-auto my-8 shadow-2xl">
            <CardHeader className="flex flex-row items-center justify-between border-b pb-4 sticky top-0 bg-white z-10">
              <div>
                <CardTitle className="flex items-center gap-2 text-gray-900">
                  ...
                  Edit Permissions - {selectedCaregiver?.fullName}
                </CardTitle>
                <p className="mt-1 text-sm text-gray-600">
            ...
            <CardContent className="p-6">
              <form onSubmit={(e) => { e.preventDefault(); handleUpdatePermissions(); }} className="space-y-6">
                {/* Permissions List */}
                <div className="space-y-4">
                  <h4 className="font-semibold">Manage Permissions</h4>
                  <div className="space-y-3">
                    {Object.entries(customPermissions).map(([key, value]) => (
                      <div key={key} className="flex items-center justify-between p-4 rounded-lg border hover:bg-gray-50">
                        <div className="flex-1">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                  <p className="text-sm text-blue-900">
                    <strong>Note:</strong> All permission changes are logged and can be audited from the Activity Log.
                  </p>
                </div>
      ...
      {viewActivityDialogOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 overflow-y-auto">
          <Card className="w-full max-w-4xl max-h-[95vh] overflow-y-auto my-8 shadow-2xl">
            <CardHeader className="flex flex-row items-center justify-between border-b pb-4 sticky top-0 bg-white z-10">
              <div>
                <CardTitle className="flex items-center gap-2 text-gray-900">
                  <Activity className="h-6 w-6" />
                  Activity Log - {selectedCaregiver?.fullName}
                </CardTitle>
                <p className="mt-1 text-sm text-gray-600">
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => setViewActivityDialogOpen(false)}
                className="h-8 w-8 p-0"
                aria-label="Close modal"
              >
                <X className="h-5 w-5" />
            ...
            <CardContent className="p-6">
              {activities.filter((a) => a.caregiverName === selectedCaregiver?.fullName).length === 0 ? (
                <div className="text-center py-12">
                  <Activity className="size-12 mx-auto mb-4 text-gray-400" />
                  <h3 className="font-semibold mb-2">No Activity Yet</h3>
                  <p className="text-gray-600">
                    No actions have been performed by {selectedCaregiver?.fullName} yet.
                  </p>
                </div>
              ) : (
                <div className="space-y-4">
                  {activities
                    ...
                    .map((activity) => (
                      <Card key={activity.activityId} className="p-4 border hover:shadow-md transition-shadow">
                        <div className="flex items-start gap-4">
                          <div className="size-12 rounded-full bg-purple-100 flex items-center justify-center text-purple-600 flex-shrink-0">
                            {getActionIcon(activity.action)}
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-start justify-between gap-4">
                              <div className="flex-1">
                                <p className="font-semibold capitalize">{activity.action.replace('_', ' ')}</p>
                                <p className="text-sm text-gray-600 mt-1">{activity.actionDetails}</p>
                                {activity.resourceName && (
